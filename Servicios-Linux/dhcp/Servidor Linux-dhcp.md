...
Después para ver nuestra ip usaremos el comando
```bash
ip addr
```
Para la configuración usaremos siempre el enp0s8 que es la ip de la red interna y ahora mismo no hay ip asignada.

Para asignar una ip debemos ir a la carpeta `netplan`.
...
Una vez actualizado hay que instalar el paquete del servicio DHCP
```bash
...
Seguido del DNS que pondremos siempre la IP de nuestro servidor para a futuro establecer este servicio, es decir si uso otro servidor pondré la IP de ese servidor.
...
Después para ver el estado del servicio tenemos dos posibles comandos:
...
Si todo ha ido bien, debería salir que está activado en verde como en la imagen.
